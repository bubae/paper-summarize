...
- **구조 기준 예시: `20260907_phan_recap.html`** (33페이지 → 2,183줄 / SVG 18개). 새 요약을 쓰기 전에 이 파일의 **절 구성·캡션 밀도·표와 그림의 분담**을 확인한다
- `20260728_kimiteam_kimik3.html`(47페이지 → 4,841줄 / SVG 55개)은 **초기 예시로, 그림 밀도는 현재 기준보다 과하다.** 서술 깊이와 캡션 품질만 참고하고 개수는 따라가지 않는다
...
구조 기준 예시: **`20260907_phan_recap.html`** (33페이지 → 2,183줄 / SVG 18개) — 현재 기준으로 쓰인 요약이다.
`20260728_kimiteam_kimik3.html`은 초기 예시로 **그림 밀도가 현재 기준보다 과하므로** 서술 깊이만 참고한다.
...
이 수치는 **하한**이지 목표가 아니다. 아래 내용 규칙을 만족하고 나서도 수가 모자라면
그때 더 그리고, 이미 넘었으면 더 그리지 않는다 — **개수를 채우려고 그림을 만들지 않는다.**

그림 개수를 정하는 것은 아래 **내용 규칙**이다 (개수 표가 아니라):

- **논문의 주요 Figure·Table은 전부 재현한다.** 골라내지 않는다
- **핵심 수식마다 수치 예제 그림 1개** — 이 포맷의 차별점
- 그림 없는 절이 3개 연속되면 밀도 부족
- 원 수치를 옮길 뿐이고 시각화가 해석을 더하지 않는 표는 **SVG 대신 `.small-table`** 로 옮긴다 (비용이 훨씬 싸다)

### 그림 유형은 다양해야 한다 (필수)

문서 전체가 "박스 + 화살표"로만 채워지면 독자가 그림을 구분하지 못한다. 다음을 지킨다:

- **서로 다른 도식 유형을 한 문서에 최소 6종** 사용한다
- **파이프라인/블록도(박스+화살표)는 전체 그림의 1/3 이하**
- 같은 유형을 **연속 3개** 쓰지 않는다
- 수치가 있으면 흐름도 대신 **차트를 먼저 고려**한다

빠른 선택 기준:

| 상황 | 쓸 도식 |
|---|---|
| before → after 쌍 비교 | 덤벨 / 슬로프 차트 |
| N개 모델 × M개 벤치마크 표 | 히트맵 |
| 합이 100%인 범주 분포 | 도넛 / 누적 막대 |
| 기여도 분해 (시작 → +a → −b → 최종) | 워터폴 |
| 두 변수의 상관·이상치 | 산점도 |
| 두 주체의 왕복 (에이전트↔환경) | 시퀀스 다이어그램 |
| 조건 분기가 핵심 | 플로차트 (마름모 분기) |
| 실제 입력/출력 문자열을 보여야 함 | **텍스트 diff 패널** (좌=잘못됨/빨강, 우=고쳐짐/초록) |
| 예시 3~6개 나열 | 카드 갤러리 |
| 핵심 결과 요약 | 숫자 카드(스코어보드) |
| 층 구조 | 계층 스택 |

전체 카탈로그(막대·레이더·깔때기·상태기계·벤 다이어그램·비유 그림·아이콘 픽토그램 등)와
SVG 작성 요령은 `paper-figures` 스킬 §6에 있다.
...
개념도 SVG를 손으로 좌표 찍어 작성하는 것이 요약 작성의 **가장 큰 병목**이다.
`.claude/skills/paper-figures/assets/svgkit.py` 에 반복 레이아웃 7종이 함수로 들어 있다:

```python
import sys; sys.path.insert(0, '.claude/skills/paper-figures/assets')
from svgkit import Svg, figure
s = Svg(800, 320, "제목", "부제", mid="uniq")   # mid = marker id 접두사(그림마다 다르게)
s.cards([("제목A","줄1","줄2"), ("제목B","줄1","줄2")], y=44, h=88, palette=["b","g"])
s.steps([("Observe","K=5 rollout"), ("Diagnose","원인 식별")], y=150, kind="p")
s.note(["결론 줄1","결론 줄2"], y=230, title="요점", kind="w")
open('/tmp/f1.svg','w').write(s.done())
```

제공 레이아웃: `cards`(카드 갤러리) · `table`(격자표/히트맵) · `steps`(가로 단계 흐름) ·
`stack`(세로 계층) · `quad`(4사분면) · `note`(결론 박스) · `scoreboard`(숫자 카드),
그리고 `box` / `text` / `arrow` / `divider` / `raw`(직접 덧그리기).
데이터 차트(꺾은선·산점도·덤벨 등)는 지금처럼 Python으로 좌표를 계산해 생성한다.
